...
void write_random_vectors(char* filename, int vec_size){
    ...
    if(!status){fprintf(stderr, "--ERRO-- Falha escrevendo tamanho do vetor no arquivo\n");exit(3);}
    ...
    for (int i=0; i < vec_size; i++){
        vec1[i] = (float) (rand() % 1000) * 0.3162898 * neg;
        vec2[i] = (float) (rand() % 1000) * 0.43216;
    ...
    status = fwrite(&dotprod, sizeof(double), 1, f);
    ...
    char debugfile_name[35];
    sprintf(debugfile_name, "testes\\debug_file%d.txt", vec_size);
